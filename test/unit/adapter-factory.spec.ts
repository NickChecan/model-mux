...
describe("AdapterFactory", () => {
  ...
  describe("createAdapter", () => {
    ...
    it("should throw an error for unsupported model prefixes", () => {
      ...
      // Act & Assert
      expect(() => AdapterFactory.createAdapter(model, baseUrl, headers, apiKey))
        .toThrow(`Unsupported model/provider: ${model}`);
    });

    it("should throw an error when model is an empty string", () => {
      // Act & Assert
      expect(() => AdapterFactory.createAdapter("", baseUrl, headers, apiKey))
